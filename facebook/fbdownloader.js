...
async function download (contentUrl) {
  ...
  await page.goto('https://fbdownloader.app/id')
  ...
  await page.waitForSelector('a[title="Download 720p (HD)"]')
  const resultUrl = await page.evaluate(() => {
    return document.querySelector('a[title="Download 720p (HD)"]').getAttribute('href')
  })
  await browser.close()
  return resultUrl
}
