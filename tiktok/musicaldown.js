...
async function download (contentUrl) {
  ...
  await page.$eval('#link_url', (el, contentUrl) => (el.value = contentUrl), contentUrl)
  await page.click('button[type="submit"]')
  await page.waitForSelector('.col.s12.l8')
  const resultUrl = await page.evaluate(() => {
    return document.querySelector('.col.s12.l8 a').getAttribute('href')
  })
  await browser.close()
  return resultUrl
}
